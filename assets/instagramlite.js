		...
		const constructMedia = function(data) {
			...
			for(let i = 0; i < data.length; i++) {
				...
				if(thisMedia.type === 'image' || thisMedia.type === 'carousel' || !s.videos) {
					...
					// construct image
					item = `<img class="il-photo__img" src="${thisMedia.images.low_resolution.url}" alt="Instagram Image" data-filter="${thisMedia.filte}" />`;
					// item = `<div style="background-image:url(${thisMedia.images.low_resolution.url});"></div>`;
				...
				if(s.list && item) {
					...
					item = `<div class="il-item" data-instagram-id="${thisMedia.id}">${item}</div>`;
				}
